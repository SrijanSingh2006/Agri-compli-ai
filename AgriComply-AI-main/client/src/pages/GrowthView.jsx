...
const GrowthView = () => {
  ...
  const handleDownloadPacket = async () => {
    try {
      setDownloading(true);
      const res = await api.get('/growth/download', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Loan_Application_Packet.zip');
      ...
      alert(err.response?.data?.error || 'Could not download loan packet');
    } finally {
    ...
    <div>
      <div className="mb-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          >
            {downloading ? 'Preparing ZIP...' : 'Download Loan Packet'}
          </button>
        </div>
      </div>
